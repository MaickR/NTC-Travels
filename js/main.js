...
(function($) {
    ...
    // Base prices
    // ========================================
    const ANDALUCIA_BASE_PRICE_EUR = 4290;
    ...
    window.updateFormFields = function() {
        ...
        } else if (type === 'booking_andalucia') {
            if (travelersGroup) travelersGroup.style.display = 'block';
            if (datesGroup) datesGroup.style.display = 'block';
            if (paymentGroup) paymentGroup.style.display = 'block';
            if (travelStyleGroup) travelStyleGroup.style.display = 'block';
            if (priorityGroup) priorityGroup.style.display = 'block';
            if (tourDatesInfo) tourDatesInfo.textContent = 'Confirmed Dates: May 5 - 16';
        } else if (type === 'booking_spain_wine') {
            if (travelersGroup) travelersGroup.style.display = 'block';
            if (datesGroup) datesGroup.style.display = 'block';
            if (paymentGroup) paymentGroup.style.display = 'block';
            if (travelStyleGroup) travelStyleGroup.style.display = 'block';
            if (priorityGroup) priorityGroup.style.display = 'block';
            if (tourDatesInfo) tourDatesInfo.textContent = 'Flexible dates - Group of 10 required';
        } 
        ...
        } else if (type === 'question_andalucia') {
            if (travelersGroup) travelersGroup.style.display = 'block';
            if (datesGroup) datesGroup.style.display = 'block';
            if (tourDatesInfo) tourDatesInfo.textContent = 'Dates: May 5 - 16';
        } else if (type === 'question_spain_wine') {
            if (travelersGroup) travelersGroup.style.display = 'block';
            if (datesGroup) datesGroup.style.display = 'block';
            if (tourDatesInfo) tourDatesInfo.textContent = 'Flexible dates - Group of 10 required';
...
Travelers: *${travelersCount || 'Not specified'}*`;
            ...
            if (numericTravelers) {
                const baseTotal = egyptPrice * numericTravelers;
                messageContent += `
Est. Total: *$${baseTotal} USD*`;
            }
...
Note: Checking availability for Private Dahabiya`;
        ...
        } else if (inquiryType === 'booking_andalucia') {
            const numericTravelers = /^\d+$/.test(travelersCount) ? parseInt(travelersCount, 10) : null;

            messageContent += `BOOKING REQUEST: Andalucía (12 Days)
----------------------------------------
Price: €${ANDALUCIA_BASE_PRICE_EUR} EUR per person (Package)
Route: Córdoba - Sevilla - Puerto de Santa María - Cádiz/Jerez - Málaga - Granada - Madrid

Travelers: *${travelersCount || 'Not specified'}*`;

            if (numericTravelers) {
                const baseTotal = ANDALUCIA_BASE_PRICE_EUR * numericTravelers;
                messageContent += `
Est. Total: *€${baseTotal} EUR*`;
            }

            messageContent += `
Note: Confirming hotels and availability for May 5 - 16`;

        } else if (inquiryType === 'booking_spain_wine') {
            messageContent += `BOOKING REQUEST: Spanish Wine Expedition
----------------------------------------
Price: $3,950 USD for group of 10 people
Hosted by: Nicolas Tena at NTC Vineyards in Spain
Experience: Wine tastings, cultural immersion, authentic gastronomy

Travelers: *${travelersCount || 'Group of 10'}*

Note: Checking available dates for private wine expedition`;
...
Client has specific questions about the Egypt itinerary.`;
            ...
            } else if (inquiryType === 'question_andalucia') {
                messageContent += `QUESTION: Andalucía Tour
        ----------------------------------------
        Client has specific questions about the Andalucía itinerary.`;

        } else if (inquiryType === 'question_spain_wine') {
            messageContent += `QUESTION: Spanish Wine Expedition
----------------------------------------
Client has specific questions about the Spanish Wine Expedition.`;
...
----------------------------------------`;
    ...
    window.handleCTAWhatsApp = function(event, tour) {
        ...
        } else if (tour === 'andalucia') {
            whatsappText = 'Hello NTC Travels! 🇪🇸\n\nI would like to reserve my spot for the Andalucía journey:';
            whatsappText += '\n• Dates: May 5 - 16';
            whatsappText += '\n• Duration: 12 Days / 11 Nights';
            whatsappText += '\n• Route: Córdoba → Sevilla → Cádiz/Jerez → Málaga → Granada → Madrid';
            whatsappText += '\n• Price: €' + ANDALUCIA_BASE_PRICE_EUR + ' EUR per person';
            whatsappText += '\n\nPlease send me availability and reservation details. Thank you!';
        } else if (tour === 'spain-wine') {
            whatsappText = 'Hello NTC Travels! 🍷🇪🇸\n\nI would like to learn more about the NTC Spanish Wine Expedition:';
            whatsappText += '\n• Hosted by: Nicolas Tena (Owner, NTC Vineyards)';
            whatsappText += '\n• Group Size: 10 travelers';
            whatsappText += '\n• Price: $3,950 USD for the entire group';
            whatsappText += '\n• Experience: Wine tastings, cultural immersion at NTC vineyards in Spain';
            whatsappText += '\n\nPlease share available dates and details. Thank you!';
        }

        if (tour !== 'egypt' && tour !== 'mxgt' && tour !== 'spain-wine') {
             whatsappText += '\nPlease share availability, payment plans, and next steps.';
        }
    ...
    window.handleCardWhatsApp = function(event, tour) {
        ...
        if (tour === 'india') {
            ...
            tourName = 'Andalucía Journey - 12 Days';
            basePrice = '€' + ANDALUCIA_BASE_PRICE_EUR + ' EUR per person (Package)';
            dates = 'May 5 - 16';

            whatsappText = `Hello NTC! I am interested in the ${tourName}\nPrice: ${basePrice}\nDates: ${dates}\nRoute: Córdoba - Sevilla - Puerto de Santa María - Cádiz/Jerez - Málaga - Granada - Madrid`;
        } else if (tour === 'spain-wine') {
            tourName = 'Spanish Wine Expedition';
            basePrice = '$3,950 USD for group of 10 people';
            dates = 'Flexible dates';

            whatsappText = `Hello NTC! I am interested in the ${tourName}\nPrice: ${basePrice}\nHosted by: Nicolas Tena at NTC Vineyards in Spain\nExperience: Wine tastings, cultural immersion, authentic gastronomy`;
            whatsappText += `\n\nPlease share available dates and complete itinerary details.`;
        }
